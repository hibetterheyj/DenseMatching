...
class ResNet(torch_resnet.ResNet):
    ...
    def modify_stride_and_padding(self, padding=''):
    ...
    def forward(self, x,  *args, **kwargs):
        ...
        x = self.layer1(x)
        x = self.layer2(x)  # output 1/8
        x = x if self.layer3 is None else self.layer3(x)  # usually stride 2, output 1/16
        x = x if self.layer4 is None else self.layer4(x)  # usually stride 2, output 1/32
