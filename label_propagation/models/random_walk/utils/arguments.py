...
def test_args():
    ...
    parser.add_argument('--topk', default=10, type=int, help='k for kNN')
    parser.add_argument(
        '--radius',
        default=12,
        type=float,
    ...
    parser.add_argument(
        '--videoLen', default=20, type=int, help='number of context frames'
    )
    ...
    parser.add_argument(
        '--filelist',
        default='/home/he/projects/DenseMatch4VOS/dense_matching/label_propagation/validation/feature_backbone_evaluation/davis_vallist.txt',
        type=str,
    )
    ...
    # Model Details
    parser.add_argument(
        '--model-type', default='scratch', type=str, help='scratch | imagenet | moco'
    )
...
def train_args():
    ...
    parser.add_argument(
        '--zero-diagonal',
        help='always zero diagonal of A',
        action="store_true",
    )
    parser.add_argument(
        '--flip',
        default=False,
        help='flip transitions (bug)',
        action="store_true",
    )

    parser.add_argument('--frame-aug', default='', type=str, help='grid or none')
    parser.add_argument(
        '--frame-transforms',
        default='crop',
        type=str,
    ...
    parser.add_argument(
        '--featdrop', default=0.0, type=float, help='"regular" dropout on features'
    )
    parser.add_argument('--restrict', default=-1, type=int, help='restrict attention')
    parser.add_argument(
        '--head-depth', default=0, type=int, help='depth of head mlp; 0 is linear'
    )
    parser.add_argument(
        '--visualize',
        default=False,
        action='store_true',
        help='visualize with wandb and visdom',
    )
        ...
        name = '-'.join(
            [
                ...
                % (
                    ...
                    else '-'.join([str(s) for s in getattr(args, k)]),
                )
                for k in keys
            ]
        )
