...
from models.competitors.random_walk.model import CRW
from models.competitors.random_walk.utils.arguments import test_args
from models.competitors.random_walk import utils as utils
from datasets.segmentation_datasets import vos
from datasets.video_datasets import jhmdb
from validation.feature_backbone_evaluation import test_utils as test_utils
from utils_data.euler_wrapper import prepare_data
from models.feature_backbones import resnet as resnet
from models.feature_backbones import From3D
from models.feature_backbones.VGG_features import VGGPyramid, VGGPyramidWithAdaptationLayer
...
def main(args, vis, settings):
    ...
    dataset = (vos.VOSDataset if not 'jhmdb' in args.filelist else jhmdb.JhmdbSet)(root=settings.env.davis, args=args)
    val_loader = torch.utils.data.DataLoader(dataset,
        batch_size=int(args.batchSize), shuffle=False, num_workers=1, pin_memory=True)
    ...
    for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta) in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            for b in range(0, imgs.shape[1], bsize):
                feat = feature_backone_model(imgs[:, b:b+bsize].transpose(1, 2).to(args.device),
