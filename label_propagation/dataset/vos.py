...
class VOSDataset(torch.utils.data.Dataset):
    ...
    # args: arguments for the dataloader
    ...
    def __getitem__(self, index):
        ...
        folder_path = os.path.join(self.root, self.jpgfiles[index])
        label_path = os.path.join(self.root, self.lblfiles[index])
        ...
        rsz_h, rsz_w = math.ceil(img.size(1) / self.mapScale[0]), math.ceil(
            img.size(2) / self.mapScale[1]
        )
        ...
        for i, p in enumerate(lbl_paths):
            ...
            if onehot is None:
                ...
                onehot = np.stack(
                    [np.all(lbls[i] == ll, axis=-1) for ll in lblset], axis=-1
                )
            ...
            if resized is None:
                ...
                resized = cv2.resize(
                    np.float32(onehot), (rsz_w, rsz_h), cv2.INTER_LINEAR
                )
            ...
            if self.texture:
                ...
                lblset = (
                    np.array(
                        [[0, 0, 0]]
                        + [cm.Paired(i)[:3] for i in range(texturized.shape[-1])]
                    )
                    * 255.0
                )
                break
